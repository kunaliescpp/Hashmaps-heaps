...
Draw a vertical line from the top to the bottom and cross the least bricks. If your line goes through the edge of a brick, then the brick
is not considered as crossed. You cannot draw a line just along one of the two vertical edges of the wall, in which case the line will 
...
public:
    int leastBricks(vector<vector<int>>& wall) {
        ...
        unordered_map<int, int>Brickedges;
        ...
        for(vector<int>row : wall){
            ...
            for(int i = 0; i< row.size()-1; i++){
                 rowsum += row[i];
                 Brickedges[rowsum]++;
                 maxedges = max(Brickedges[rowsum], maxedges);
            }
        }
